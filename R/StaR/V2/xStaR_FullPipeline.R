...
for(curAnalysis in 1:2)
{
  ...
  print("nbPoints - Trouve un moyen de lire le file ou regarder le workspace?")
  if(bERSP)
  {
    nbPoints = 100*200#54000
  }
  if(bERP)
  {
    nbPoints = 1536
  ...
  if(bLoadMatlabFile)
  {
    if(bERP)
    {
      ...
      matlabData <- staR_fillFromMatlab("~/Documents/PhD/Stats Test/mTBI_SubClean_Measures/MPT_Export/MPT_exp_erp_D1.mat", "MPT", fullData, bSmallSamples, nbPoints)
    }
    if(bERSP)
    {
      #matlabData <- staR_fillFromMatlab("~/Documents/Playground/UdeM/RMatlab_Data/export_mpt.mat", "MPT", fullData, nbPoints)
      matlabData <- staR_fillFromMatlab("~/Documents/PhD/Stats Test/mTBI_SubClean_Measures/MPT_Export/MPT_exp_ersp_D1.mat", "MPT", fullData, bSmallSamples, nbPoints)
    }
